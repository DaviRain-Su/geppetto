...
Agent 读 `testing.rs`，知道用 litesvm 或 mollusk-svm 搭建测试环境，构建交易、模拟执行、验证状态变更。
...
| feature gates | CPI helpers 作为 optional features，`default = []` 按需启用。提供 `token-all`（token + token-2022 + ata）和 `full`（全部）预设组合 |
...
| 扩展第二批 | `assert_readonly`, `assert_system_program`, `assert_token_program`, `assert_current_program`, `assert_account_count`, `assert_ata` |
...
| 项目   | 说明                                           |
| ---- | -------------------------------------------- |
| 模块   | `testing.rs` — litesvm/mollusk-svm 测试惯用法     |
| 版本头  | 同 FR-5：geppetto 版本、pinocchio 版本、日期、Solana 版本 |
...
**核心定位：Built as a harness component for AI coding agents.**

> "2026 年 AI coding 的进化：Prompt Engineering → Context Engineering → **Harness Engineering**。Skills 和文档是 context（喂更好的上下文），Geppetto 是 harness（工程化改造环境）。AccountSchema 是编译期约束，guard 是 mechanical enforcement，doctest 是自动验证。每次 agent 犯错，不是再提示一次，而是让它**以后不可能再犯同样的错**。Geppetto 是 Solana 生态第一个 agent harness 组件。"

**精简版（30 秒 pitch）：**

> "Anchor 和 Quasar 让人类写代码更快。Geppetto 让 AI agent 写代码更可靠——不是通过更好的 prompt，而是通过编译期约束和可验证知识。它是 Pinocchio 的 agent harness。"
