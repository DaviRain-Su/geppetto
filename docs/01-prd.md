...
开发者运行 `cargo generate --git https://github.com/DaviRain-Su/geppetto-template`，输入项目名。生成的 monorepo 包含 `program/`（Rust 合约骨架 + Geppetto 依赖）和 `app/`（Next.js + @solana/kit 前端骨架）。Agent 打开项目，读到 AGENTS.md，知道：Rust 侧读 geppetto doc comments，TypeScript 侧的 `idl/index.ts` 必须与 `state.rs` 对齐。在骨架上直接填充业务逻辑，从零到全栈可编译项目只需一行命令。

### 场景 1b：在已有项目中添加 Geppetto

开发者在已有 Pinocchio 项目中 `cargo add geppetto`。Agent 读到 AGENTS.md，知道从 `geppetto` 而非 `pinocchio` 导入，使用 `AccountSchema` 定义账户，用 `guard::*` 做安全检查，按 `dispatch` 模式分发指令。产出代码一次通过安全审查。
...
| 内容 | 告诉 agent：1) 训练数据过时，读 doc comments；2) 所有账户必须实现 `AccountSchema`；3) 所有安全检查必须用 `guard::*`；4) 指令分发用标准模式 |
...
### FR-10：cargo-generate 全栈模板（P1 — 重要）

| 项目 | 说明 |
|---|---|
| 描述 | `cargo-generate` monorepo 模板，一行命令生成从合约到前端的完整项目 |
| 用法 | `cargo generate --git https://github.com/DaviRain-Su/geppetto-template` |
| 生成结构 | **program/**：`Cargo.toml`（geppetto 依赖）、`src/{lib,processor,state,error}.rs`、`src/instructions/mod.rs`、`tests/integration.rs`（litesvm）。**app/**：`package.json`（@solana/kit + Next.js）、`src/idl/index.ts`（程序 ID + 账户布局契约）、`src/hooks/useProgram.ts`、Next.js App Router 页面。**根目录**：`AGENTS.md`（统一指引，Rust 节 + TypeScript 节）、`docs/03-technical-spec.md` |
| 契约对齐 | `app/src/idl/index.ts` 的账户布局偏移量必须与 `program/src/state.rs` 的 `AccountSchema` 精确匹配。AGENTS.md 明确告知 agent 这个关系 |
| 前端策略 | 用 solana-dev-skill 推荐的官方栈（@solana/kit + Next.js），Geppetto 不封装前端，只确保前后端接口对齐 |
| 验收标准 | `cargo generate` 后：`program/` 下 `cargo build` 通过；`app/` 下 `npm install && npm run build` 通过；agent 打开项目后无需额外指引即可开始开发 |
| 约束 | 不写自研 CLI，纯模板文件 + `cargo-generate.toml` 配置。独立仓库 |
| 工作量 | 1-2 天（核心在 idl/index.ts 和 AGENTS.md；首次搭建 Next.js + @solana/kit 环境预留缓冲） |
...
| 自研 CLI 脚手架 | 不做 `geppetto init` CLI，用 `cargo-generate` 模板替代（FR-10） |
...
| P1 | FR-10 cargo-generate 全栈模板 | 是（1 天工作量，demo 叙事关键环节） |

## 与竞品的差异化定位

Geppetto vs `solana-dev-skill`：

- `solana-dev-skill` = 建议（纯 markdown，广而浅，Anchor 默认）
- Geppetto = 约束（代码 + 知识合一，深而专，Pinocchio-first）
- 两者互补：模板前端侧直接使用 solana-dev-skill 推荐的官方栈（@solana/kit + Next.js），Geppetto 专注合约侧深度知识 + 前后端契约对齐
