...
> 状态：已完成（当前范围：E3-01 ~ E3-03 已交付并验证；E3-04 ~ E3-08 保留为后续增强项）
...
**E3 完成定义（DoD）**：
- [x] 知识模块版本头可自动检查；
- [ ] 多 agent 入口文件可自动检查是否漂移；（后续增强）
- [ ] `docs/03-technical-spec.md` 的关键 feature matrix 可与 `Cargo.toml` 自动比对；（后续增强）
- [ ] 仓库提供统一的一致性检查入口命令；（后续增强）
