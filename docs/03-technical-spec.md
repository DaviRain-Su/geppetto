...
> **说明（实现 vs 下游使用）**：本节与后续 `schema.rs` / `testing.rs` 等代码块，
> 描述的是 **Geppetto 自身源码的技术规格**。
> 因此其中出现的 `pinocchio::*` 引用是 **内部实现细节**，不是下游推荐导入方式。
> 面向 Geppetto 使用者的公开示例，应统一写成 `geppetto::*`、
> `geppetto::token::*`、`geppetto::log::*`、`geppetto::pubkey::*`。
...
| 常量                      | 值                                              | 用途                      |
| ----------------------- | ---------------------------------------------- | ----------------------- |
| `SYSTEM_PROGRAM_ID`     | `11111111111111111111111111111111`             | assert\_system\_program |
| `SPL_TOKEN_PROGRAM_ID`  | `TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`  | assert\_token\_program  |
| `TOKEN_2022_PROGRAM_ID` | `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`  | assert\_token\_program  |
| `ATA_PROGRAM_ID`        | `ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL` | assert\_ata             |
...
### 5.2 设计说明：为什么用 split\_tag + match 而不是 handlers 数组
...
**注意**：此模块必须兼容 `#![no_std]`。不使用 `std::error::Error`，不使用 `thiserror`。对 Geppetto 内部实现而言，仅依赖 `core` 与底层 `pinocchio::error::ProgramError`。
...
| 批次 | 话题                                                              | 黑客松必须 |
| -- | --------------------------------------------------------------- | ----- |
| P0 | Entrypoint 选择 + no\_allocator/nostd                             | 是     |
| P0 | 账户切片解构                                                          | 是     |
...
- 选择规则：system CPI 用简单风格，token CPI 优先使用 `geppetto::token` / `geppetto::token_2022` re-export 的结构体 `.invoke()` 方法
...
- CPI 时根据实际 token program 选择对应的 `geppetto::token` 或 `geppetto::token_2022`（或在共享指令上直接使用 `geppetto::token_2022` 的显式 `token_program` 参数）
...
- `geppetto::hint::likely()` / `geppetto::hint::unlikely()`
- 用在热路径 vs 错误路径的条件判断
- token 程序大量使用
...
// 所有日志调用都应该在 cfg 门控后
#[cfg(feature = "log")]
geppetto::log::log!("Processing create instruction");
```
...
## 8. src/anti\_patterns.rs — 反模式（FR-5，纯文档）
...
所有模块（guard、schema、dispatch、idioms、anti\_patterns、client、testing）的第一行 doc comment 必须包含：
...
| 12 | `rent_exempt_minimum` data\_len = 0                | 返回 `128 * 3480 * 2 = 890880`         |
