...
} from '../../lib/e7-delivery-packet'

function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'geppetto-e8-'))
}

function removeDir(directory: string): void {
  fs.rmSync(directory, { recursive: true, force: true })
}

function writeFixtures(directory: string, override: { actionPlan?: string; draft?: string; checklist?: string } = {}): void {
