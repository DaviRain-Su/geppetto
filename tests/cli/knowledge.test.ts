...
import { checkKnowledgeHeaders } from '../../lib/knowledge-check'
...
function createTempRepo({ cargoToml, files }: TempRepoOptions): string {
...
function createManifest(root: string, relativePath: string, ecosystem = 'pinocchio'): Array<Required<KnowledgeTarget>> {
  return [{
    relativePath,
