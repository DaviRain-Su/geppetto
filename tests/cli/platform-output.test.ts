...
import * as output from '../../lib/platform/output'
...
function makeState(overrides: Record<string, any> = {}) {
  return {
    ...
    program_path: overrides.program_path || 'examples/escrow',
    program_binary: overrides.program_binary || 'target/deploy/escrow.so',
...
test('renderDeployOutput JSON format matches contract', () => {
  const state = makeState()
  let written = ''
  const stream = { write: (s: string) => { written += s } }
...
test('writeArtifacts throws EDEPLOY005 on artifact fs failures', () => {
  const tempDir = createTempDir()

  try {
    const state = makeState()
    const blockedRoot = path.join(tempDir, 'repo-root-file')
    fs.writeFileSync(blockedRoot, 'not-a-directory', 'utf8')

    assert.throws(() => {
      output.writeArtifacts(state, blockedRoot)
    }, (error: any) => {
...
function writeManifest(tempDir: string, programId = '') {
  const manifest = [
    'schema_version = "0.1"',
    '',
    '[app]',
...
function writeManifestWithoutProgramId(tempDir: string) {
  const manifest = [
    'schema_version = "0.1"',
    '',
    '[app]',
