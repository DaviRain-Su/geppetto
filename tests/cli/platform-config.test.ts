...
import { parseGeppettoToml, loadPlatformConfig } from '../../lib/platform/config'
...
function writeManifest(root: string, extra: Record<string, string> = {}) {
...
test('parseGeppettoToml reads top-level and section values', () => {
  ...
  assert.equal((parsed as any).schema_version, '0.1')
  assert.equal((parsed as any).app.name, 'escrow-demo')
  assert.equal((parsed as any).solana.cluster, 'devnet')
})
...
test('applyOverrides applies allowlisted values with correct precedence shape', () => {
  ...
  try {
    ...
    assert.equal((next as any).solana.cluster, 'testnet')
    assert.equal((next as any).solana.programId, 'abc123')
    assert.equal((next as any).offchain.encoreApp, 'renamed-api')
    assert.equal((next as any).deploy.replicas, 3)
    assert.equal(config.solana.cluster, 'devnet')
  } finally {
    removeDir(tempDir)
  }
