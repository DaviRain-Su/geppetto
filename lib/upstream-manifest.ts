...
export type UpstreamSourceType = 'cargo' | 'cargo-lock' | 'knowledge-header' | 'knowledge-table'

export interface UpstreamManifestSource {
...
export interface UpstreamManifestEntry {
  ...
  dependencyName: string
  description: string
  sources: UpstreamManifestSource[]
}

export const UPSTREAM_TRACKING_MANIFEST: readonly UpstreamManifestEntry[] = Object.freeze([
  {
...
export const ALLOWED_SOURCE_TYPES: readonly UpstreamSourceType[] = Object.freeze([
  'cargo',
  'cargo-lock',
  'knowledge-header',
  'knowledge-table',
...
export function getUpstreamManifestRoot(): string {
  return path.resolve(__dirname, '..')
}

export function getUpstreamTrackingManifest(root = getUpstreamManifestRoot()): UpstreamManifestEntry[] {
...
export function assertUpstreamTrackingManifest(
  ...
  for (const item of manifest) {
    ...
    for (const source of item.sources) {
      if (!ALLOWED_SOURCE_TYPES.includes(source.sourceType)) {
        throw new Error(`Unsupported source type ${source.sourceType} in ${item.dependencyName} (${item.upstreamName})`)
      }
