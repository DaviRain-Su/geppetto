...
const ALLOWED_DEPLOY_MODES = new Set(['hybrid'])
...
function buildPlatformConfig(parsed, manifestPath) {
  ...
  const offchain = parsed.offchain || {}
  ...
  const config = {
    ...
    deploy: {
      mode: assertAllowedValue(assertRequiredString(deploy.mode, 'deploy.mode'), 'deploy.mode', ALLOWED_DEPLOY_MODES),
